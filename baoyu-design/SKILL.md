...
- The user wants a **slide deck / presentation / PPT** (quarterly report, pitch deck, training slides, status review) → read [`built-in-skills/make-a-deck.md`](built-in-skills/make-a-deck.md). For a **quarterly business review**, also read [`references/quarterly-deck-template.md`](references/quarterly-deck-template.md) for the proven 15-slide structure. For PPTX export after the deck is built, read [`built-in-skills/export-as-pptx-editable.md`](built-in-skills/export-as-pptx-editable.md).
...
## PPTX Export Troubleshooting
When `gen-pptx` export fails due to environment issues (HTTP server cannot start, Playwright not installed, etc.), provide the user with fallback options in this order:
1. **Browser Print to PDF** — open the deck in browser, Ctrl+P, set landscape, enable background graphics, save as PDF. This preserves all visual styling.
2. **Screenshot + PowerPoint** — use browser devtools or a screenshot tool to capture each slide at 1920×1080, then insert as background images in blank PowerPoint slides.
3. **Retry with fixed environment** — if the user can install missing dependencies (e.g., `npx playwright install chromium`, or a stable HTTP server like `npx serve`), retry the automated export.

### Windows Hermes Environment Pitfall
On Windows Hermes, the `terminal` tool blocks shell backgrounding (`&`) and `background=true` processes may exit immediately (exit code 49). This prevents starting a local HTTP server for `gen-pptx`. **Do not loop on server startup attempts** — after 2-3 failed tries, immediately fall back to browser print-to-PDF. Workarounds (try each once, then give up):
- **Use `execute_code` with `subprocess.Popen`** — Python's `http.server` or Node's `npx serve` launched via `execute_code` may survive where `terminal` kills them.
- **If all server attempts fail**, fall back to browser print-to-PDF immediately rather than looping on server startup.
